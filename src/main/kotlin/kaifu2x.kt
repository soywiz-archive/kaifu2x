...
class GraphBuilder(val g: Graph) {
	...
	inline fun <TR> session(callback: Session.() -> TR): TR {
		return Session(g).auto { callback(it) }
	}

	inline fun <TR> Session.runner(callback: Session.Runner.() -> TR): TR {
		return callback(runner())
	}

	fun <T> constant(name: String, value: Any, type: Class<T>): UnnamedOutput<T> {
...
fun tfdemo() {
	graph {
		...
		val const1 = constant(value.toByteArray(UTF8))
		val const2 = constant(value.toByteArray(UTF8))

		session {
			runner {
				fetchRunFirst(const1 + const2).auto { output ->
